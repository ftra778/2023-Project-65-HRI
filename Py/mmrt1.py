...
if webcam_cap == 0:
    cap = cv2.VideoCapture(r"/afs/ec.auckland.ac.nz/users/f/t/ftra778/unixhome/Documents/videos/Videos/" + name_of_save + r".mp4")
else:
    ...
    if (time_elapsed > 1./frame_rate) or (webcam_cap):
        ...
        if cap_fail == 1:
            ...
            if np.linalg.norm(RS_LS[:]) - shoulders > 0:    # If the shoulders are closer to the camera, it indicates a lean forward, and vice versa NEW
                phi = phi * (-2.5)                            # Double value since forward leaning detection doesn't work properly
